    ...
    if choice == 'EDA':
        ...
        if data is not None:
            ...
            if st.checkbox('Pie Plot'):
                ...
                column_to_plot = st.selectbox('Select 1 Column',
                        all_columns)
                pie_plot = \
                    df[column_to_plot].value_counts().plot.pie(autopct='%1.1f%%'
                        )
                st.write(pie_plot)
                st.pyplot()
    elif choice == 'Plots':
        ...
        if data is not None:
            ...
            selected_columns_names = \
                st.multiselect('Select Columns To Plot',
                               all_columns_names)
        ...
        if data is not None:
            ...
            X = df.iloc[:, 0:-1]
            Y = df.iloc[:, -1] # edit the target - able to submit the target as selected variable
            seed = 7

            # prepare models

            models = []
            models.append(('LogRression', LogisticRegression()))
            models.append(('LinearDiscriminantAnalysis', LinearDiscriminantAnalysis()))
            models.append(('KNeighborClass', KNeighborsClassifier()))
            ...
            for (name, model) in models:
                ...
                accuracy_results = {'model name': name,
                                    'model_accuracy': cv_results.mean(),
                                    'standard deviation': cv_results.std()}
                all_models.append(accuracy_results)
            if st.checkbox('Metrics As Table'):
                st.dataframe(pd.DataFrame(zip(model_names, model_mean,
                             model_std), columns=['Algo',
                             'Mean of Accuracy', 'Std']))
